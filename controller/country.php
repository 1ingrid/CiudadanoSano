<?php
    ...
    if($jwt->verificar($token, 'countries')) {
        switch ($_SERVER['HTTP_ACCION']) {
            ...
            break;
            case 'registro':
                $resultado = $country->nuevo($_POST);
                echo json_encode($resultado);
            break;
            case 'modificar':
                $resultado = $country->actualizar(file_get_contents("php://input"));
                echo json_encode($resultado);
            break;
            case 'desactivar':
                $resultado = $country->desactivar(file_get_contents("php://input"));
                echo json_encode($resultado);
            break;
            case 'activar':
                $resultado = $country->activar(file_get_contents("php://input"));
                echo json_encode($resultado);
            break;
